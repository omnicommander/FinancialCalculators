...
function singleDepositComputeForm () {
  if (
    ...
    var e = parseInt(
        $('#singleDepositPayments')
          .val()
          .replace(',', '')
      ),
      t = parseFloat(
        $('#singleDepositInterest')
          .val()
          .replace(',', '')
      ),
      a = parseFloat(
        $('#singleDepositPrincipal')
          .val()
          .replace(',', '')
      )
    if (1 > e > 480) alert('Years to Accrue must be between 1 and 480')
    else if (0.001 > t > 99) alert('Interest rate must be between 0.001 and 99')
  ...
  if (
    ...
    if ('' != $('#multipleDepositInterest').val())
      if ('' != $('#multipleDepositMoAdd').val())
        if ('' != $('#multipleDepositMoAdd').val()) {
          var e = parseInt(
              $('#multipleDepositPayments')
                .val()
                .replace(',', '')
            ),
            t = parseFloat(
              $('#multipleDepositInterest')
                .val()
                .replace(',', '')
            ),
            a = parseFloat(
              $('#multipleDepositPrincipal')
                .val()
                .replace(',', '')
            ),
            n = parseFloat(
              $('#multipleDepositMoAdd')
                .val()
                .replace(',', '')
            ),
            o = t
