...
// 	var amount = document.getElementById('loanAffordAmount').value;
// 	var payment = document.getElementById('loanAffordPayment').value;
// 	var rate = document.getElementById('loanAffordRate').value;
// 	var months = document.getElementById('loanAffordMonths').value;

// 	if (isNaN(amount)) { (alert('Amount must be a number!'), (amount.focus)) };
// 	if (isNaN(payment)) { (alert('Payment must be a number!'), (payment.focus)) };
// 	if (isNaN(rate)) { (alert('Rate must be a number!'), (rate.focus)) };
// 	if (isNaN(months)) { (alert('Months must be a number!'), (months.focus)) };
// }
...
function validateFormLoanAfford() {
	...
	return temparray = e.split(o),
		...
		isNaN(e)
			...
			$("#loanAffordAmount").focus(), !1)
		: isNaN(t)
			? (alert("Payment must be a number!"), $("#loanAffordPayment").focus(), !1)
			: isNaN(a) ? (alert("Rate must be a number!"), $("#loanAffordRate").focus(), !1)
				: isNaN(n) ? (alert("Length of Loan must be a number!"), $("#loanAffordMonths").focus(), !1)
					: 0 != e && "" != e && 0 != t && "" != t ? (alert("Please clear either the loan payment or the loan amount to continue."), $("#loanAffordAmount").focus(), !1)
						: 0 != e && "" != e || 0 != t && "" != t ? 0 == a || "" == a
							? (alert("You must select a loan rate!"), $("#loanAffordRate").focus(), !1)
							: (a /= 1200, 0 == n || "" == n
								? (alert("You must provide the term of the loan!"), $("#loanAffordMonths").focus(), !1)
								: ("1" == $("#loanAffordFrequency").val() && (n *= 12), 0 == t || "" == t
									? $("#loanAffordPayment").val(parseInt(100 * (e * (a / (1 - Math.pow(1 + a, -n))) + .005)) / 100)
									: $("#loanAffordAmount").val(parseInt(100 * (Math.pow(1 + a, -n) * (-t + Math.pow(1 + a, n) * t) / a + .005)) / 100), !1))
